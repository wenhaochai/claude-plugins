...
4. **Spacing: title, legend, plot must sit level and clear.** `header_legend` re-pads the title by `8 + rows*(legend_size+4.5)`; when sibling panels have different header row counts, re-pad the shorter ones with the max rows so all titles sit level. After any header change, view the render and check nothing overlaps or clips.
