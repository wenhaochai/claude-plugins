...
3. Before the full render, draw test frames at every scene boundary and busy moment, tile them into a contact sheet, and look for overlaps, clipped text and dead time. A hidden browser tab may freeze mid-render and the app may stop idle preview servers, so if frames stop arriving, restart the servers, reload the page and resume from the last saved frame.
